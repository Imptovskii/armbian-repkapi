...
/path/to/build/ - это путь до папки со сборщиком

```
sudo kpartx -av /path/to/build/armbian-repkapi/output/images/<Образ>.img
```

Затем вам программа kpartx выстаст название устройства напимер /dev/loop1p1. Только у вас будет уже своя цифра (/dev/loop1p1, /dev/loop2p1 и т.д.).
